...
uint8_t* read_entire_file(const char* filename) {
	...
	if (size == 0) {
		LOG_MSG(error, "%s had a size of 0 bytes, skipping\n", filename);
		return NULL;
	}
	...
	if (file == NULL) {
		LOG_MSG(error, "Unable to open %s\n", filename);
		return NULL;
	}
	...
	if (file_data == NULL) {
		fclose(file);
		LOG_MSG(error, "Unable to allocate %d bytes for %s\n", size, filename);
		return NULL;
	}
	...
	LOG_MSG(info, "Read %d bytes in from %s\n", size, filename);
...
bool dll_inject_memory_file(uint32_t pid, const char* path) {
	uint8_t* dll = read_entire_file(path);
	if (dll == NULL) {
...
bool dll_inject_memory(uint32_t pid, uint8_t* dll_data) {
	...
	while (import_dir->Name > 0) {
		...
		LoadLibrary(module_name); // Make sure we have a copy
		uintptr_t remote_handle = (uintptr_t)get_remote_module(module_name, pid); // Address in remote process
		...
		while (*lookup != 0) {
			bool use_ordinal = is_ordinal(*lookup);
			uint16_t ordinal = lookup_ordinal(*lookup);
			uint32_t func_name_rva = lookup_name_rva(*lookup);
			...
			if (use_ordinal) {
				...
				void* proc_addr = get_remote_proc_addr(module_name, func_name, pid);
...
bool remote_load_library(uint32_t pid, const char* dll_path) {
	// Try to open the target process with required permissions
	HANDLE remote = OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_CREATE_THREAD, false, pid);
	if (remote == NULL) {
		LOG_MSG(error, "Failed to open target process %d\n", pid);
		return false;
	}
	...
	// Get &LoadLibraryA()
	void* load_library = get_remote_proc_addr("KERNEL32.dll", "LoadLibraryA", pid);
	...
	// Sometimes the thread never exits, and freeing the memory before LoadLibrary()
	// can run will crash it. So I'm just letting the DLL path memory leak.
